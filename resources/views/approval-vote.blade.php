                        ...
                        <table class="table table-bordered table-responsive-lg table-striped table-hover">
                            ...
                            <tbody class="products">
                                @foreach ($allApprovalVote as $item)
                                    <tr>
                                        <td><a href="{{ route('list.detail-approval-vote', ['id' => $item->Id]) }}">{{$item->DeptName}}</a></td>
                                        <td><a href="{{ route('list.detail-approval-vote', ['id' => $item->Id]) }}">{{$item->DocNo}}</a></td>
                                        <td>{{date('d-m-Y', strtotime($item->DocDate))}}</td>
                                        <td>{{$item->Description}}</td>
                                        <td>{{$item->DocStatusName}}</td>
                                        <td><button type="button" class="btn btn-info update-status-approval-vote" id="{{$item->Id}}">Duyệt phiếu</button></td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
