            ...
            foreach ($request->ProductCode as $item => $productCode) {
                ...
                DB::insert('EXEC usp_Create_B30JobRecord ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?', 
                    [
                        ...
                        isset($request->DocDate[$item]) ? $request->DocDate[$item] : config('constants.null')
                    ]
                );
            }
            DB::commit();
            return response()->json(['error_correct' => $request->DocDate === config('constants.null') ? __('messages.product_order.success') : __('messages.product_order.additional')]);
        } catch (\Exception $e) {
            DB::rollBack();
